class User < ApplicationRecord
  ...
  with_options presence: true do
    ...
    validates :first_name,
              format: { with: /\A[ぁ-んァ-ン一-龥]/, message: 'is invalid. Kanji / Hiragana / Katakana input characters.' }
    validates :last_name,
              format: { with: /\A[ぁ-んァ-ン一-龥]/, message: 'is invalid. Kanji / Hiragana / Katakana input characters.' }
  ...
  validates :password,
            format: { with: /\A(?=.*?[a-z])(?=.*?\d)[a-z\d]+\z/i,
                      message: 'Encrypted password is invalid. Please enter alphanumeric characters.' }
end
